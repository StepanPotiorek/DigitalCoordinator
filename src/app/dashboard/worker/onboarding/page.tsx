...
export default function WorkerOnboardingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">{t("dashboard.myOnboarding", lang)}</h1>
        <p className="mt-1 text-sm text-slate-400">
          {t("dashboard.trackOnboarding", lang)}
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6 backdrop-blur-sm">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-sm text-slate-400">
            {t("dashboard.completedOf", lang).replace("{done}", String(completed)).replace("{total}", String(total))}
          </span>
      ...
      {Object.entries(grouped).length === 0 ? (
        <p className="text-sm text-slate-400">{t("dashboard.noOnboardingItems", lang)}</p>
      ) : (
